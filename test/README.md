...
### 3. `automated_generation_e2e_test.ts`
**コンテンツ生成E2Eテスト（CI/CD対応）**
...
```bash
# TypeScriptファイルを直接実行
npx ts-node test/automated_generation_e2e_test.ts

# または事前にコンパイルして実行
npx tsc test/automated_generation_e2e_test.ts
```

**テスト内容:**
1. 複数のテストJSONファイル（test_no_audio.json等）で新規プロジェクト作成
2. JSONエディタでのファイル編集（パス修正、beat削除）
3. コンテンツ生成の実行と完了待機
4. 生成されたコンテンツの再生テスト

**特徴:**
- ✅ 自動でElectronアプリを起動・終了
- ✅ CI/CD環境での使用が可能  
- ✅ data-testidベースの安定したセレクタ使用
- ✅ 問題のあるパスの自動修正（画像URLへの変換、オーディオbeatの削除）
- ✅ 長時間実行に対応（生成完了まで待機）

### 4. `manual_run_test_several_files.ts`
**手動実行用複数ファイルテスト**

- 手動で起動済みのElectronアプリに接続
- 複数のJSONファイルを使用してコンテンツ生成テスト
- 開発時のテスト用途に最適
...
# 2. 別のターミナルでテストを実行
npx ts-node test/manual_run_test_several_files.ts
```

**テスト内容:**
- automated_generation_e2e_test.ts と同じ内容
- アプリの起動・終了は手動で行う
...
### 5. `manual_run_switch_language.ts`
**言語切り替え機能テスト**
...
以下のテストファイルがCI/CD環境で使用できます：

### 基本テスト
`.github/workflows/playwright_test.yml` で `automated_e2e_test.ts`:
```yaml
- name: Run Basic E2E tests (with xvfb)
  run: |
...
### コンテンツ生成テスト
`.github/workflows/content_generation_test.yml` で `automated_generation_e2e_test.ts`:
```yaml
- name: Run Content Generation tests (with xvfb)  
  run: |
    xvfb-run -a npx ts-node test/automated_generation_e2e_test.ts
  env:
    CI: true
    NODE_ENV: test
  timeout-minutes: 30  # 生成処理のため長めのタイムアウト
```

**CI環境での特徴:**
- ✅ `xvfb-run`でHeadless実行
- ✅ 環境変数でCI環境を検知
- ✅ 自動的なアプリ起動・終了
- ✅ 適切なタイムアウト設定
