  ...
  <Card class="p-4">
    ...
    <div class="space-y-3">
      <div>
        ...
        <div class="mb-2 text-xs text-muted-foreground">{{ t("parameters.captionParams.languageDescription") }}</div>
      ...
      <div>
        ...
        <div class="mb-2 text-xs text-muted-foreground">{{ t("parameters.captionParams.stylesDescription") }}</div>
        <Textarea
          v-model="styles"
          :placeholder="`${t('ui.common.example')}\ncolor: #FF6B6B;\nfont-family: 'Arial Black', sans-serif;\ntext-shadow: 2px 2px 4px rgba(0,0,0,0.5);`"
          :class="['font-mono', { 'cursor-not-allowed bg-muted text-muted-foreground': !props.captionParams?.lang }]"
          rows="6"
